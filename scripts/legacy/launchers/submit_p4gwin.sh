...
source "$ROOT_DIR/scripts/legacy/launchers/p4g.sh"
...
stage1_id="$(sbatch --parsable --partition="$PARTITION" --gres="gpu:${S1_GPUS}" --cpus-per-task="$S1_CPUS" --mem="$S1_MEM_MB" --export="$common_export" "$ROOT_DIR/scripts/legacy/launchers/job_p4s1.sbatch")"
stage2_id="$(sbatch --parsable --partition="$PARTITION" --dependency="afterok:${stage1_id}" --array="$array_spec" --gres="gpu:${S2_GPUS}" --cpus-per-task="$S2_CPUS" --mem="$S2_MEM_MB" --export="$common_export" "$ROOT_DIR/scripts/legacy/launchers/job_p4s2.sbatch")"
