#!/usr/bin/env python3
"""Preserve relative-noise control and launch corrected geometry and decay from scratch."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'outputs/church-joint-geometry-20260912')
    args=p.parse_args()
    output=args.output.resolve();output.mkdir(parents=True,exist_ok=True)
    for variant in ['joint']:
    ...
    for pid in set(args.pause_pids+[args.credential_pid]):
        ...
        expected_script = 'train_church_relative_noise.py' if pid == args.credential_pid else 'train_church_ffhq_noise.py'
        assert str(ROOT/'scripts'/expected_script) in command
        assert (process/'cwd').resolve()==ROOT
        training=Path(command[command.index('--output')+1])
        expected_output = ROOT/('outputs/church-relative-noise-20260911/relative' if pid == args.credential_pid else 'outputs/church-ffhq-noise-20260911/calibrated')
        assert training == expected_output
        sources[pid]={'pid':pid,'output':str(training),'status_before':json.loads((training/'status.json').read_text())}
    assert args.credential_pid not in args.pause_pids
    ...
    for pid in args.pause_pids:
        ...
        saved=torch.load(training/'last.pt',map_location='cpu',weights_only=False,mmap=True)
        ...
        del saved
    (output/'paused-fixed-noise.json').write_text(json.dumps(list(sources.values()),indent=2)+'\n')
    ...
    launches=[]
    for gpu,variant in [(0, 'joint')]:
        run_id='church-joint-geometry-20260912'
        command=[sys.executable,'-u',str(ROOT/'scripts/train_church_joint_geometry.py'),
                 '--output',str(output/variant),'--variant','control',
                 '--calibration',str(output/'calibration/calibration.json'),'--wandb-id',run_id]
