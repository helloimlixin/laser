#!/usr/bin/env python3
"""Submit a +15 epoch checkpointed continuation for W&B run u3aoy68w.

The local run is ImageNet stage-1 k2-a4096 from
imagenet-full-stage1-sweep-20260626_132835.  The latest completed local
checkpoint is the second segment's ``last.ckpt`` at epoch 6/global step 71177.
This launcher extends the run from 7 to 22 total epochs.  The default resource
shape keeps the original effective batch size: 3 GPUs, per-GPU batch 21, and
gradient accumulation 2, so the LR remains the batch-adjusted 4e-5.
"""
...
RUN_ID = "u3aoy68w"
WANDB_PROJECT = "laser"
WANDB_NAME = "imagenet-stage1-k2-a4096-imagenet-full-stage1-sweep-20260626_132835"
...
RUN_ROOT_BASE = Path("/scratch/xl598/runs/imagenet_stage1_sweep_continue15_u3aoy68w")
DATA_DIR = Path("/scratch/xl598/Projects/data/imagenet")
CKPT = Path(
    "/scratch/xl598/runs/imagenet_stage1_sweep/"
    "imagenet-full-stage1-sweep-20260626_132835/k2-a4096/stage1/"
    "checkpoints/run_slurm57327129_0/laser/last.ckpt"
)
...
def stage1_overrides(args, output_dir):
    ...
    return [
        ...
        (
            "wandb.tags="
            f"[stage1,imagenet,laser,full,sweep,continuation,continue15,gpu-redhat,"
            f"{int(args.nodes)}node,{total_gpus(args)}gpu,effbatch{eff_batch}]"
        ),
...
def write_job_files(job_root, snapshot, args):
    ...
    log_base = job_root / "u3aoy68w_continue15"
    run_script = job_root / "run_u3aoy68w_continue15.sh"
    sbatch_script = job_root / "sbatch_u3aoy68w_continue15.sh"
    ...
    run_script.write_text(
        textwrap.dedent(
            ...
            export TMPDIR="/tmp/laser_u3aoy68w_continue_${{SLURM_JOB_ID:-$$}}"
            export TEMP="$TMPDIR"
            export TMP="$TMPDIR"
...
def parse_args():
    ...
    parser.add_argument("--job-name", default="imnet-u3aoy68w-cont")
...
def main():
    ...
    snapshot = snapshot_repo(
        args.repo,
        args.snapshot_root,
        f"laser_u3aoy68w_stage1_continue15_{stamp}",
    )
