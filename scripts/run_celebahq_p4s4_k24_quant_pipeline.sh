#!/usr/bin/env bash
# CelebA-HQ p4s4/k24 quantized-token pipeline.
#
# This follows the promising stage-2 formulation from W&B run nro3eadh:
# 16x16 latent -> non-overlapping 4x4 latent patches -> 4x4 patch grid,
# sparse depth k=24, and 256 quantized coefficient bins. Stage 2 therefore
# models a 4x4x48 interleaved atom/coeff token grid instead of raw coefficients.
set -euo pipefail

cd /home/xl598/Projects/laser

STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="${RUN_ROOT:-/home/xl598/Projects/laser/runs/celebahq_p4s4_k24_quant_${STAMP}}"
...
WANDB_GROUP="${WANDB_GROUP:-celebahq_p4s4_k24_quant_${STAMP}}"
...
echo "[$(date --iso-8601=seconds)] p4s4/k24 quant pipeline start"
echo "RUN_ROOT=${RUN_ROOT}"
echo "STAGE1_DIR=${STAGE1_DIR}"
echo "STAGE2_DIR=${STAGE2_DIR}"
echo "CACHE=${CACHE}"
echo "WANDB_GROUP=${WANDB_GROUP}"
echo "PATCH=${PATCH_SIZE}/${PATCH_STRIDE}  SPARSITY_LEVEL=${SPARSITY_LEVEL}  COEFF_BINS=${COEFF_BINS}"
...
if [[ -z "${STAGE1_CKPT}" ]]; then
  "${PYTHON_BIN}" train_stage1_autoencoder.py \
    ...
    wandb.name="celebahq_s1_p4s4_k24_lpips_${STAMP}"
...
"${PYTHON_BIN}" train_stage2_prior.py \
  ...
  wandb.project="${WANDB_PROJECT}" \
  wandb.group="${WANDB_GROUP}" \
  wandb.name="celebahq_s2_p4s4_k24_q${COEFF_BINS}_${STAMP}" \
  wandb.save_dir="${STAGE2_DIR}/wandb"

echo "[$(date --iso-8601=seconds)] p4s4/k24 quant pipeline done: ${RUN_ROOT}"
