#!/usr/bin/env python3
"""Pause the mistaken Church recipe and launch archived FFHQ-v4 scratch experiments."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'outputs/church-ffhq-archived-20260911')
    args=p.parse_args()
    ...
    for pid in set(args.pause_pids+[args.credential_pid]):
        ...
        assert str(ROOT/'scripts/train_church_original_scratch.py') in command
    ...
    for pid in args.pause_pids:
        ...
        del saved
    (output/'previous-wrong-recipe-pause.json').write_text(json.dumps(list(sources.values()),indent=2)+'\n')
    ...
    for gpu,variant in enumerate(['looped','control']):
        run_id=f'church-ffhq-archived-{variant}-20260911'
        command=[sys.executable,'-u',str(ROOT/'scripts/train_church_ffhq_archived.py'),
