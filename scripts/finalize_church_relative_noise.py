#!/usr/bin/env python3
"""Verify bounded magnitude-relative noise and fresh training before launch."""
import json
...
BASE = ROOT/'outputs/church-relative-noise-20260911'
...
def main():
    ...
    assert int(suite.attrib['tests']) == 15
    ...
    old_path = ROOT/'outputs/church-ffhq-noise-20260911/calibrated'
    ...
    excluded = {'output', 'wandb_id', 'stop_after_step', 'fid_samples', 'confirmation_samples',
                ...
                'comparison_control', 'intentional_training_change'}
    common = (original.keys() & config.keys()) - excluded
    assert all(original[k] == config[k] for k in common)
    assert original.keys()-config.keys() == {'coefficient_noise_sigma_physical', 'coefficient_noise_temperature_physical', 'normalized_temperatures_per_depth'}
    assert config.keys()-original.keys() == {'coefficient_noise_sigma_cap', 'relative_sigma', 'truncate', 'coefficient_noise_rule'}
    ...
    assert str(ROOT/'scripts/train_church_ffhq_noise.py').encode() in cmd
    ...
    sources = set(config['source_hashes']) | {
        'scripts/calibrate_church_relative_noise.py', 'scripts/launch_church_relative_noise.py',
        'scripts/finalize_church_relative_noise.py', 'scripts/finalize_church_ffhq_archived.py',
        'tests/test_church_relative_noise.py', 'tests/test_church_ffhq_archived.py',
        'docs/lsun-church-relative-noise-2026-09-11.md', 'src/rqvae_metrics.py',
        'scripts/audit_church_noise_scale.py', 'src/church_coefficient_noise.py',
        'src/models/rqtransformer/configs.py', 'src/models/rqtransformer/primitives.py',
        str(calibration_path.relative_to(ROOT)),
    }
    result = {'production_ready': True, 'unit_tests_passed': 15,
