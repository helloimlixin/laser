#!/usr/bin/env bash
set -Eeuo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PYTHON_BIN="${PYTHON_BIN:-/workspace/tmp/laser-h100-venv/bin/python}"
STAGE1="$ROOT/outputs/lsun-church-a16384-k4-fair-stage1-b256-20260815-064515/church256-rqvae-laser-8x8-a16384-k4-fair/15082026_064530/best_rfid_slot1_model.pt"
STAGE2_INIT="$ROOT/outputs/lsun-church-a16384-k4-compound-v7-pairhard-ft15-seed0-20260818/checkpoints/best_fid_17.6033_epoch_015.pt"
DATA="$ROOT/outputs/lsun-church-a16384-k4-compound-v5-stage2-20260816-052724/data/lsun"
CACHE="$ROOT/outputs/lsun-church-a16384-k4-causal-prefix-patterns-v1-20260818/token_cache/lsun_church_train_a16384k4_prefix_patterns_512_1024_2048_4096.pt"
FID_STATS="$ROOT/third_party/rq-vae-transformer/assets/fid_stats/lsun_256_church.npz"
CONTINUOUS_RFID="$ROOT/outputs/lsun-church-a16384-k4-cache-rfid-preflight/continuous.json"
QUANTIZED_RFID="$ROOT/outputs/lsun-church-a16384-k4-cache-rfid-preflight/quantized.json"
PATTERN_RFID="$ROOT/outputs/lsun-church-a16384-k4-coefficient-pattern-v4096-20260818/rfid/full-pattern-126227-native.json"
OUTPUT="${RUN_ROOT:-$ROOT/outputs/lsun-church-a16384-k4-causal-prefix-pattern-v2-pairhard-ft15-seed0-20260818}"
WANDB_ID="${WANDB_RUN_ID:-lsunchurch-prefix-pattern-v2-pairhard-ft15-seed0-20260818}"
WANDB_MODE="${WANDB_MODE:-online}"
NPROC="${NPROC:-4}"
STAGE2_BATCH_SIZE="${STAGE2_BATCH_SIZE:-64}"
EPOCHS="${EPOCHS:-15}"
FID_EVERY="${FID_EVERY:-5}"
MAX_OPTIMIZER_STEPS="${MAX_OPTIMIZER_STEPS:-0}"

for required in "$PYTHON_BIN" "$STAGE1" "$STAGE2_INIT" "$DATA" "$CACHE" \
  "$FID_STATS" "$CONTINUOUS_RFID" "$QUANTIZED_RFID" "$PATTERN_RFID"; do
  [[ -e "$required" ]] || { echo "missing required input: $required" >&2; exit 1; }
done

mkdir -p "$OUTPUT/checkpoints"
init_args=()
if [[ ! -f "$OUTPUT/checkpoints/last.pt" ]]; then
  init_args=(--init-stage2-checkpoint "$STAGE2_INIT")
fi

export PYTHONPATH="$ROOT/third_party/rq-vae-transformer:$ROOT${PYTHONPATH:+:$PYTHONPATH}"
export NCCL_NVLS_ENABLE="${NCCL_NVLS_ENABLE:-0}"
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-8}"
export WANDB_DIR="${WANDB_DIR:-$ROOT/wandb}"
export LASER_CHECKPOINT_STAGING_DIR="${LASER_CHECKPOINT_STAGING_DIR:-/tmp/laser-checkpoint-staging}"

exec "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node="$NPROC" \
  "$ROOT/scripts/train_official_rqtransformer_laser_stage2.py" \
  --checkpoint "$STAGE1" --data "$DATA" --dataset lsun_church \
  --model-preset lsun-church-350m --token-cache "$CACHE" \
  --cache-rfid-preflight "$CONTINUOUS_RFID" "$QUANTIZED_RFID" "$PATTERN_RFID" \
  --output "$OUTPUT" --checkpoint-dir "$OUTPUT/checkpoints" \
  "${init_args[@]}" \
  --distributed-backend ddp --epochs "$EPOCHS" --batch-size "$STAGE2_BATCH_SIZE" \
  --total-batch-size 256 --max-optimizer-steps "$MAX_OPTIMIZER_STEPS" \
  --num-atoms 16384 --sparsity-level 4 --coeff-vocab-size 2048 --coeff-max 3 \
  --causal-prefix-pattern-tokens --compound-micro-transformer-layers 2 \
  --atom-loss-weight 1.5 --pattern-loss-weight 1 \
  --seed 0 --lr 0.0001 --lr-schedule cosine --lr-schedule-epochs "$EPOCHS" \
  --min-lr 0.00001 \
  --atom-temperature 1 --atom-top-k 0 --atom-top-p 0.92 \
  --coeff-temperature 1 --coeff-top-k 0 --coeff-top-p 0.92 \
  --fid-num-samples 50000 --fid-batch-size 250 --fid-every "$FID_EVERY" \
  --metric-backend original-rqvae --fid-reference-stats "$FID_STATS" \
  --save-ckpt-freq 5 --keep-best-checkpoints 1 --model-only-best-checkpoints \
  --save-step-freq 0 --sample-grid-every 1000 \
  --sample-grid-size 64 --sample-grid-samples-per-class 8 \
  --sample-grid-batch-size 8 --sample-grid-seed 0 \
  --resume --wandb-mode "$WANDB_MODE" --wandb-entity helloimlixin-rutgers \
  --wandb-project laser --wandb-id "$WANDB_ID" \
  --wandb-name "lsun-church-causal-prefix-pattern-v2-pairhard-ft15-seed0-20260818"
