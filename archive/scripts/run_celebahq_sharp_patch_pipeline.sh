#!/usr/bin/env bash
# CelebA-HQ patch-token sharpness pipeline.
#
# This retrains stage 1 with a patch dictionary so stage 2 sees a much shorter
# sparse token grid: 16x16 latent sites become 8x8 non-overlapping 2x2 patches.
set -euo pipefail

cd /home/xl598/Projects/laser

STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT="${RUN_ROOT:-/home/xl598/Projects/laser/runs/celebahq_sharp_patch_pipeline_${STAMP}}"
...
WANDB_GROUP="${WANDB_GROUP:-celebahq_sharp_patch_pipeline_${STAMP}}"
...
echo "[$(date --iso-8601=seconds)] patch pipeline start"
...
"${PYTHON_BIN}" train.py stage1 \
  ...
  wandb.name="celebahq_s1_patch_d4_lpips_${STAMP}"
...
"${PYTHON_BIN}" train.py stage2 \
  ...
  wandb.project="${WANDB_PROJECT}" \
  wandb.group="${WANDB_GROUP}" \
  wandb.name="celebahq_s2_patch_d4_lpips_${STAMP}" \
  wandb.save_dir="${STAGE2_DIR}/wandb"

echo "[$(date --iso-8601=seconds)] patch pipeline done: ${RUN_ROOT}"
