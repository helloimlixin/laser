#!/usr/bin/env bash
# Launch p4s4/k8 LASER transformer-prior jobs across CelebA-HQ, FFHQ,
# ImageNet, and VCTK. This follows the promising p2s2/k4/q256 stage-2 run
# jp1vzn1q, but doubles sparse depth while halving the image patch grid.
...
RUN_TAG="${RUN_TAG:-p4s4k8-jp1vzn1q-more-$(date +%Y%m%d_%H%M%S)}"
RUN_ROOT_BASE="${RUN_ROOT_BASE:-/scratch/$USER/runs/laser_p4s4k8_multidata_transformer}"
...
# Longer than the 30/150 defaults used by the earlier multidata launcher.
...
# Interleaved atom/coeff token depth is 16, so flattened length is 256 tokens.
...
DICTIONARY_USAGE_GRAD_SCALE="${DICTIONARY_USAGE_GRAD_SCALE:-0.5}"
DICTIONARY_USAGE_GRAD_MIN="${DICTIONARY_USAGE_GRAD_MIN:-0.25}"
DICTIONARY_USAGE_GRAD_MAX="${DICTIONARY_USAGE_GRAD_MAX:-4.0}"
...
ONLINE_KSVD_MAX_SAMPLES="${ONLINE_KSVD_MAX_SAMPLES:-1024}"
ONLINE_KSVD_MAX_ATOMS="${ONLINE_KSVD_MAX_ATOMS:-512}"
ONLINE_KSVD_BLEND="${ONLINE_KSVD_BLEND:-0.10}"
...
# Match jp1vzn1q's larger transformer.
...
# VCTK waveform: 32768 samples / (8*8*8) = 64 latent steps. With p4s4/k8
# this also gives 16 patch sites and 256 interleaved atom/coeff tokens.
...
submit_image_dataset() {
  local dataset="$1"
  local recipe_label="dictfocus-p${PATCH_SIZE}s${PATCH_STRIDE}k${SPARSITY_LEVEL}-a${NUM_EMBEDDINGS}-q${COEFF_BINS}-cm${COEF_MAX}-cc${COMMITMENT_COST}-bw${BOTTLENECK_LOSS_WEIGHT}-s1e${STAGE1_EPOCHS}-adve${STAGE1_ADV_EPOCHS}-s2e${STAGE2_EPOCHS}"
  "$PYTHON_SUBMIT" scripts/submit_multimodal_sweep.py \
    ...
    --stage1-override model.dictionary_update_mode="$DICTIONARY_UPDATE_MODE" \
...
submit_vctk_audio() {
  local recipe_label="dictfocus-ds512-p${VCTK_PATCH_SIZE}s${VCTK_PATCH_STRIDE}k${VCTK_SPARSITY_LEVEL}-a${VCTK_NUM_EMBEDDINGS}-q${COEFF_BINS}-cm${COEF_MAX}-cc${VCTK_COMMITMENT_COST}-bw${VCTK_BOTTLENECK_LOSS_WEIGHT}-s1e${STAGE1_EPOCHS}-adve${STAGE1_ADV_EPOCHS}-s2e${STAGE2_EPOCHS}"
  "$PYTHON_SUBMIT" scripts/submit_multimodal_sweep.py \
    "${COMMON_ARGS[@]}" \
    --gpus "$AUDIO_GPUS" \
...
echo "=== p4s4/k8 multidata transformer sweep ==="
...
echo "image recipe: p${PATCH_SIZE}s${PATCH_STRIDE} k${SPARSITY_LEVEL} a${NUM_EMBEDDINGS} q${COEFF_BINS}"
echo "vctk recipe: downsample=$VCTK_DOWNSAMPLE_RATES p${VCTK_PATCH_SIZE}s${VCTK_PATCH_STRIDE} k${VCTK_SPARSITY_LEVEL} a${VCTK_NUM_EMBEDDINGS} q${COEFF_BINS}"
echo "dictionary: mode=$DICTIONARY_UPDATE_MODE coef_max=$COEF_MAX image_cc=$COMMITMENT_COST image_bw=$BOTTLENECK_LOSS_WEIGHT usage_grad=${DICTIONARY_USAGE_GRAD_SCALE}[${DICTIONARY_USAGE_GRAD_MIN},${DICTIONARY_USAGE_GRAD_MAX}] online_refresh=$ONLINE_KSVD_ENABLED every=$ONLINE_KSVD_INTERVAL_STEPS blend=$ONLINE_KSVD_BLEND"
echo "wandb visuals: image/audio every ${VIS_LOG_EVERY_N_STEPS} steps, diagnostics every ${DIAG_LOG_INTERVAL} steps, codebook vectors=$DICTIONARY_VIS_MAX_VECTORS"
