...
#
# Fresh run:
#   DATA_DIR=/data/celebahq_packed_256 RUN_ROOT=$PWD/runs/local_p4 bash scripts/legacy/launchers/local_p4.sh
#
# Stage-2 only from an existing run root:
#   MODE=s2 RUN_ROOT=$PWD/runs/local_p4 WIN_LIST=32 bash scripts/legacy/launchers/local_p4.sh
#
# Stage-2 only with explicit refs:
#   MODE=s2 RUN_ROOT=$PWD/runs/local_p4 \
#   S1_CKPT=/path/to/last.ckpt CACHE_PT=/path/to/tok_q256.pt \
#   WIN_LIST=32 bash scripts/legacy/launchers/local_p4.sh
...
case "$MODE" in
  all|s1)
    bash "$ROOT_DIR/scripts/legacy/launchers/run_p4s1.sh"
    ;;
  s2)
    ;;
  *)
...
if [[ "$MODE" == "all" || "$MODE" == "s2" ]]; then
  ...
  for win in "${wins[@]}"; do
    ...
    WIN="$win" bash "$ROOT_DIR/scripts/legacy/launchers/run_p4s2.sh"
  done
fi
