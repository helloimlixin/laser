#!/usr/bin/env bash
# Closed-loop autoregressive coefficient fine-tune for LSUN Church. Training
# unrolls each four-deep sparse site and feeds the model-predicted gamma state
# into the next atom decision; sampling and the 256-position layout are unchanged.
...
STAGE2_INIT="$ROOT/outputs/lsun-church-a16384-k4-orthogonal-causal-stage2-20260822/stage2/checkpoints/best_fid_18.6020_epoch_010.pt"
...
RUN_ROOT="${RUN_ROOT:-$ROOT/outputs/lsun-church-a16384-k4-orthogonal-closed-loop-stage2-20260822}"
OUTPUT="$RUN_ROOT/stage2"
WANDB_ID="${WANDB_RUN_ID:-lsunchurcha16384k4orthogonalclosedloop-20260822}"
...
common_args=(
  ...
  --closed-loop-coeff-state straight-through
  --closed-loop-gumbel-temperature 1
...
active_phase=closed_loop_smoke
if [[ ! -f "$RUN_ROOT/.smoke_complete" ]]; then
  status "$active_phase" starting "full checkpoint transfer plus one closed-loop optimizer step"
  "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node=1 \
    ...
    --wandb-mode disabled \
    2>&1 | tee "$RUN_ROOT/logs/closed-loop-smoke.log"
  touch "$RUN_ROOT/.smoke_complete"
  status "$active_phase" complete "one closed-loop optimizer step passed"
fi

active_phase=closed_loop_stage2
status "$active_phase" starting "10 epochs; straight-through gamma state; 50k FID at epoch 10"
init_args=()
...
exec "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node=1 \
  ...
  --wandb-name "lsun-church-orthogonal-closed-loop-ft10-20260822" \
