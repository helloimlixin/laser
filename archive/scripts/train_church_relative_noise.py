#!/usr/bin/env python3
"""Train the archived FFHQ/Church baseline with bounded magnitude-relative noise."""
import argparse
import codecs
import gc
import json
...
def main():
    ...
    config = json.loads(json.dumps({**vars(args), 'parameters': sum(p.numel() for p in model.parameters()),
        ...
        'comparison_control': 'helloimlixin-rutgers/laser/church-ffhq-physical-noise-20260911',
        'intentional_training_change': 'only coefficient target/context noise: magnitude-relative width and bounded tails',
        ...
        'objective': 'same archived compound_objective: atom weight1.5, bounded magnitude-relative soft coefficients, geometry .05 delayed2/ramp3 epochs',
        ...
        'source_hashes': {name: sha256_file(ROOT/name) for name in [
            'scripts/train_church_relative_noise.py', 'src/church_relative_noise.py',
    ...
    if args.wandb_id:
        ...
        wb = wandb.init(entity='helloimlixin-rutgers', project='laser', id=args.wandb_id,
            name=f'Church FFHQ v4 bounded relative sigma={relative} cap={sigma} SCRATCH', group='church-relative-noise-20260911',
