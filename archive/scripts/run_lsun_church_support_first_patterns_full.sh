...
CACHE="$ROOT/outputs/lsun-church-a16384-k4-coefficient-pattern-v4096-20260818/token_cache/lsun_church_train_a16384k4_coeff_patterns_v4096.pt"
...
OUTPUT="${RUN_ROOT:-$ROOT/outputs/lsun-church-a16384-k4-support-first-pattern-v1-e300-seed0-20260818}"
WANDB_ID="${WANDB_RUN_ID:-lsunchurch-support-first-pattern-v1-e300-seed0-20260818}"
...
for required in "$PYTHON_BIN" "$STAGE1" "$DATA" "$CACHE" "$FID_STATS" \
  "$CONTINUOUS_RFID" "$QUANTIZED_RFID" "$PATTERN_RFID"; do
...
exec "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node="$NPROC" \
  ...
  --support-first-pattern-tokens --atom-loss-weight 1 --pattern-loss-weight 4 \
  --seed 0 --lr 0.0005 --lr-schedule cosine --lr-schedule-epochs "$EPOCHS" \
  --min-lr 0 \
  ...
  --wandb-name "lsun-church-support-first-pattern-v1-e300-seed0-20260818"
