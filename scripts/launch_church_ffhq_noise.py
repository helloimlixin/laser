#!/usr/bin/env python3
"""Preserve the FFHQ control, pause looping, and launch a calibrated-noise scratch comparison."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'outputs/church-ffhq-noise-20260911')
    args=p.parse_args()
    ...
    for pid in set(args.pause_pids+[args.credential_pid]):
        ...
        assert str(ROOT/'scripts/train_church_ffhq_archived.py') in command
    ...
    for pid in args.pause_pids:
        ...
        del saved
    (output/'paused-looped.json').write_text(json.dumps(list(sources.values()),indent=2)+'\n')
    ...
    launches=[]
    for gpu,variant in [(0, 'calibrated')]:
        run_id='church-ffhq-physical-noise-20260911'
        command=[sys.executable,'-u',str(ROOT/'scripts/train_church_ffhq_noise.py'),
