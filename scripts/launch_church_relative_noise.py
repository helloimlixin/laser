#!/usr/bin/env python3
"""Preserve fixed-sigma training, save the old noisy control, and launch relative-noise scratch training."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'outputs/church-relative-noise-20260911')
    args=p.parse_args()
    ...
    for pid in set(args.pause_pids+[args.credential_pid]):
        ...
        expected_script = 'train_church_ffhq_noise.py' if pid == args.credential_pid else 'train_church_ffhq_archived.py'
        ...
        expected_output = ROOT/('outputs/church-ffhq-noise-20260911/calibrated' if pid == args.credential_pid else 'outputs/church-ffhq-archived-20260911/control')
    ...
    for pid in args.pause_pids:
        ...
        del saved
    (output/'paused-legacy-control.json').write_text(json.dumps(list(sources.values()),indent=2)+'\n')
    ...
    launches=[]
    for gpu,variant in [(1, 'relative')]:
        run_id='church-relative-noise-20260911'
        command=[sys.executable,'-u',str(ROOT/'scripts/train_church_relative_noise.py'),
