...
from church_official_fid import save_rgb,score_saved_rgb
...
def main():
    ...
    screen_root=out/'official-reconstruction'
    if rank==0:
        (screen_root/'originals').mkdir(parents=True,exist_ok=False)
        (screen_root/'reconstructions').mkdir()
    dist.barrier()
    original_records,reconstruction_records=[],[]
    ...
    for images,ids in loader:
        ...
        original_records.append(save_rgb(screen_root/'originals',real,rank,int(ids[0])))
        reconstruction_records.append(save_rgb(screen_root/'reconstructions',decoded,rank,int(ids[0])))
    ...
    gathered=[None]*world
    dist.all_gather_object(gathered,dict(originals=original_records,reconstructions=reconstruction_records))
    dist.all_reduce(pixel)
    assert token_hash==state_sha256(tokenizer)
    if rank==0:
        official=ROOT/'outputs/lsun-church-bar-20260910/assets/lsun_256_church.npz'
        original_records=[row for shard in gathered for row in shard['originals']]
        reconstruction_records=[row for shard in gathered for row in shard['reconstructions']]
        status('official_reconstruction_evaluation',images=screen_n)
        _,real_mu,real_cov=score_saved_rgb(screen_root/'originals',original_records,screen_n,official,device)
        refscore,fake_mu,fake_cov=score_saved_rgb(screen_root/'reconstructions',reconstruction_records,screen_n,official,device)
        a,b=(screen_n,real_mu,real_cov),(screen_n,fake_mu,fake_cov)
        ...
        atomic_json(out/'reconstruction-screen.json',dict(images=screen_n,matched_rfid=matched,
            evaluator='released rqvae.metrics.fid: file-based Inception, dense statistics and Frechet distance',
        ...
        for folder,records in [(screen_root/'originals',original_records),(screen_root/'reconstructions',reconstruction_records)]:
            for row in records:(folder/row['file']).unlink()
        atomic_json(screen_root/'rgb-cleanup.json',dict(completed=True,retained='Inception features/statistics and RGB hashes'))
