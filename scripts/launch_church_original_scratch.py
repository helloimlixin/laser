#!/usr/bin/env python3
"""Stop mistaken checkpoint continuations and launch the original recipe from scratch."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'outputs/church-original-scratch-20260911')
    args=p.parse_args()
    ...
    for pid in set(args.pause_pids+[args.credential_pid]):
        ...
        assert str(ROOT/'scripts/train_church_epoch50_loop.py') in command
    ...
    for pid in args.pause_pids:
        ...
        del saved
    (output/'previous-continuation-pause.json').write_text(json.dumps(list(sources.values()),indent=2)+'\n')
    ...
    for gpu,variant in enumerate(['looped','control']):
        run_id=f'church-original-scratch-{variant}-20260911'
        command=[sys.executable,'-u',str(ROOT/'scripts/train_church_original_scratch.py'),
