...
stage2_id="$(sbatch --parsable --partition="$PARTITION" --array="$array_spec" --gres="gpu:${S2_GPUS}" --cpus-per-task="$S2_CPUS" --mem="$S2_MEM_MB" --export="$common_export" "$ROOT_DIR/scripts/legacy/launchers/job_p4s2.sbatch")"
