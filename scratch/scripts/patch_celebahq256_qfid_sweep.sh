...
OUT_ROOT="${OUT_ROOT:-/scratch/$USER/runs/celebahq256_patch_qfid_sweep}"
...
for num_atoms in "${num_atoms_list[@]}"; do
  for sparsity_level in "${sparsity_level_list[@]}"; do
    run_name="celebahq256_qfid_p${PATCH_SIZE}s${PATCH_STRIDE}_a${num_atoms}_k${sparsity_level}"
    job_name="chq256-qfid-a${num_atoms}-k${sparsity_level}"
