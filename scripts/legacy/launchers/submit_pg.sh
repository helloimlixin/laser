...
source "$ROOT_DIR/scripts/legacy/launchers/pg.sh"
...
for tag in "${cases[@]}"; do
  ...
  s1_id="$(sbatch --parsable --partition="$PARTITION" --job-name="${tag}-s1" --export="$common_export" "$ROOT_DIR/scripts/legacy/launchers/job_ps1.sbatch")"
  s2_id="$(sbatch --parsable --partition="$PARTITION" --job-name="${tag}-s2" --dependency="afterok:${s1_id}" --array="$array_spec" --export="$common_export" "$ROOT_DIR/scripts/legacy/launchers/job_ps2.sbatch")"
