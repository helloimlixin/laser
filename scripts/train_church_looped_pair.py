#!/usr/bin/env python3
"""Matched recurrent versus unrolled compound RQ-Transformer Church pilot."""
import argparse
import codecs
import gc
import json
...
from scripts.train_official_rqtransformer_laser_stage2 import (
    LaserAux, atomic_torch_save, scheduled_geometry_weight,
)
...
from scripts.train_church_calibrated import objective
...
def evaluate(model, aux, data, args):
    ...
    for first in range(0, len(data['atoms']), 16):
        ...
        _, metrics = objective(model, aux, atoms, physical, 'hard', .05, .05, stochastic=False)
...
def main():
    ...
    p.add_argument('--variant', choices=['looped', 'unrolled'], required=True)
    ...
    size = 2048
    cache_hash = sha256_file(args.cache)
    ...
    model = looped_pair_prior(args.variant, args.dropout).cuda()
    ...
    config = json.loads(json.dumps({**vars(args), 'architecture': 'balanced',
        ...
        'target_coordinate': 'existing scalar coefficient bins; deterministic nearest bin in depth-normalized coordinates',
        'coefficient_vocabulary': size, 'cache_sha256': cache_hash, 'stage1_sha256': stage1_hash,
        'coefficient_pattern_constraint': False, 'train_images': stream.size,
        ...
        'depth_events': 4, 'head_effective_layers': 6, 'head_unique_layers': 2 if args.variant == 'looped' else 6,
        'head_passes': 3 if args.variant == 'looped' else 1,
        'initial_function': 'same seed; first two depth blocks copied cyclically; control copies train independently',
        ...
        'objective': 'hard scalar targets; atom weight 1.5; coefficient weight 1; distribution geometry .05 delayed 2 epochs and ramped over 3',
        'depth_context': 'original compound full-pair causal head; coefficient micro-transformer additionally consumes current atom; signed physical contributions plus learned pair embeddings',
        ...
        'holdout_context': 'deterministic nearest scalar bins; completed pairs condition both spatial and depth streams',
        'source_hashes': {name: sha256_file(ROOT / name) for name in [
            'scripts/train_church_looped_pair.py', 'src/church_looped_pair.py', 'scripts/train_church_calibrated.py',
            'src/church_calibrated_training.py',
    ...
    if args.wandb_id:
        ...
        wb = wandb.init(entity='helloimlixin-rutgers', project='laser', id=args.wandb_id,
            name=f'Church compound RQ {args.variant}; six effective depth blocks', group='church-looped-pair-20260911', job_type='compound-rq-depth-loop-comparison',
    ...
    def log(row):
        ...
        write_json(args.output / 'status.json', {'pid': os.getpid(), 'target_mode': args.variant,
    ...
    log({'phase': 'ready', 'parameters': config['parameters'], 'coefficient_bins': size, 'resumed': args.resume})
    try:
        ...
        while stream.epoch + stream.position / stream.size < args.epochs and not early_stop:
            for images, actual_indices in loader:
                ...
                for micro, batch in enumerate(images.split(args.microbatch)):
                    ...
                    loss, metrics = objective(model, aux, atoms.long(), physical, 'hard', .05, weight_geo, stochastic=False)
                ...
                if step == 1 or step % 10 == 0 or epoch_end:
                    log({'phase': 'train', **{f'train/{k}': v for k, v in totals.items()},
                        'train/lr': lr, 'train/geometry_weight': weight_geo, 'train/gradient_norm': float(norm),
