...
def test_shapes_and_grads():
    torch.manual_seed(0)
    model = Encoder(in_channels=3, num_hiddens=128, num_residual_blocks=2, num_residual_hiddens=32)

    x = torch.randn(4, 3, 256, 256, requires_grad=True)
    y = model(x)

    assert y.shape == (4, 128, 64, 64)
...
def test_vis_random():
    ...
    grid = _make_activation_grid(y, max_channels=16)
    out_file = ARTIFACTS_DIR / 'encoder_activations.png'
    torchvision.utils.save_image(grid, str(out_file))
...
def test_celeba():
    ...
    model = Encoder(in_channels=3, num_hiddens=128, num_residual_blocks=2, num_residual_hiddens=32).to(device)
    model.eval()

    out_dir = ARTIFACTS_DIR / 'celeba'
    ...
    with torch.no_grad():
        for i, (images, _) in enumerate(loader):
            ...
            act_grid = _make_activation_grid(feats, max_channels=16)
            torchvision.utils.save_image(act_grid, str(out_dir / f'activations_{i:04d}.png'))
    ...
    assert (out_dir / 'activations_0000.png').exists()
...
def test_imagenet():
    ...
    _run_dir_viz(path, out_subdir='imagenet', image_size=256)
...
def test_ffhq():
    ...
    _run_dir_viz(path, out_subdir='ffhq', image_size=1024)
