#!/usr/bin/env python3
"""Sample generations from maintained stage-2 transformer checkpoints."""
...
if sys.version_info < (3, 10):
    raise SystemExit(
        "ERROR: sample.py requires Python >= 3.10. "
...
def main():
    p = argparse.ArgumentParser(description="Sample generations from maintained stage-2 transformer checkpoints.")
    p.add_argument("--ckpt", "--checkpoint", "--stage2-checkpoint", dest="checkpoint", type=Path, default=None, help="Stage-2 transformer Lightning checkpoint (.ckpt).")
    ...
    direct = save_grid(out.imgs, out_dir, nrow=pick_nrow(int(args.num_samples), args.nrow))
    dump = None
