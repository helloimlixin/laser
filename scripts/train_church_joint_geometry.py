#!/usr/bin/env python3
"""Train compound Church tokens with atom-conditional geometry and early LR decay."""
import argparse
import codecs
import gc
import json
...
from src.church_ffhq_archived import (initialization_audit, FullBatchEpochStream,
    full_training_cache, ARCHIVE_SHA256, UPSTREAM_CONFIG)
...
def main():
    ...
    parser.add_argument('--variant', choices=['control'], required=True)
    ...
    early_decay_lr(0, args.epochs, 1, peak=args.lr)
    ...
    sigma = float(calibration['sigma_cap'])
    relative = float(calibration['selected_relative_sigma'])
    truncate = float(calibration['truncate'])
    ...
    assert calibration['target_code_sha256'] == sha256_file(ROOT/'src/church_relative_noise.py')
    ...
    aux = RelativeChurchAux(args.stage1, 16384, 2048, 3., coeff_scales=scales,
                   sparsity_level=4, soft_target_physical=False, sigma_cap=sigma, relative_sigma=relative, truncate=truncate).cuda().eval().requires_grad_(False)
    ...
    config = json.loads(json.dumps({**vars(args), 'parameters': sum(p.numel() for p in model.parameters()),
        ...
        'coeff_scales': scales, 'coeff_max': 3., 'coefficient_noise_sigma_cap': sigma, 'relative_sigma': relative, 'truncate': truncate,
        'coefficient_noise_rule': 'sigma(c)=min(cap,relative*abs(c)); truncate at 3 sigma; nearest-bin fallback',
        'calibration_sha256': sha256_file(args.calibration),
        'comparison_control': 'helloimlixin-rutgers/laser/church-relative-noise-20260911',
        'intentional_training_change': 'correct candidate-conditional geometry and early LR decay; retain relative-noise tokens',
        'coeff_targets': 'bounded magnitude-relative soft distributions with sampled contexts; same bins',
        'sampler': 'archived sampler: atom top-k250/p1; coefficient p0.85; both temperatures1',
        'lr_schedule': 'cosine 5e-4 to 5e-5 over first 10 epochs, then cosine to 1e-6 by epoch300; zero warmup',
        ...
        'objective': 'archived classification atom weight1.5; candidate-conditional joint geometry .05 delayed2/ramp3 epochs; same bounded coefficient targets',
        ...
        'source_hashes': {name: sha256_file(ROOT/name) for name in [
            'scripts/train_church_joint_geometry.py', 'src/church_joint_geometry.py', 'src/church_relative_noise.py',
    ...
    if args.wandb_id:
        ...
        wb = wandb.init(entity='helloimlixin-rutgers', project='laser', id=args.wandb_id,
            name='Church joint conditional geometry + early decay SCRATCH', group='church-joint-geometry-20260912',
    ...
    try:
        ...
        while stream.epoch + stream.position/stream.size < args.epochs:
            ...
            step += 1
            lr = early_decay_lr(step-1, config['maximum_steps'], config['steps_per_epoch'], peak=args.lr)
