"""Interleaved independent atom/coefficient fields with complete 100-bit site integers."""
import math
...
from src.complete_sparse_codec import pack_exact
from src.models.rqtransformer.transformers import RQTransformer


class InterleavedPairRQTransformer(RQTransformer):
    """Eight causal events: a0,c0,a1,c1,a2,c2,a3,c3; one 100-bit stored code."""
    cached_hidden = ChurchPatternOrderRQTransformer.cached_hidden

    def __init__(self, config, num_atoms, coefficient_vocabulary=2048):
        config = config.copy()
    ...
    def pack(self, atoms, coefficient_ids):
        return self.pack_fields(atoms, coefficient_ids)
...
def interleaved_pair_prior(dropout=.15):
    ...
    return InterleavedPairRQTransformer(config, 16384)


@torch.no_grad()
def nearest_coefficient_ids(aux, physical):
    normalized = physical.float()/aux.coeff_scales
    return torch.bucketize(normalized.contiguous(), (aux.coeff_bins[:-1]+aux.coeff_bins[1:])/2)


def pack_complete_sites(atoms, coefficients):
    if atoms.shape != coefficients.shape or atoms.shape[-1] != 4:
        raise ValueError('Expected matching four-pair site tensors')
    return [pack_exact(a,c) for a,c in zip(atoms.reshape(-1,4).tolist(),coefficients.reshape(-1,4).tolist())]


def interleaved_pair_objective(model, aux, atoms, physical):
    ids = nearest_coefficient_ids(aux, physical)
    ...
    loss = joint.mean()/8  # Mean NLL of all eight scalar fields; joint NLL logged separately.
    with torch.no_grad():
        ...
        metrics = {'loss':float(loss.detach()),'atom_nll':float(atom_nll.mean()),
            'coefficient_nll':float(coefficient_nll.mean()),'joint_nll':float(joint.mean()),
