...
def record_signature(cache):
    """Includes utterance identity, text, split, and actual temporal lengths."""
    records = [{**{k: v for k, v in r.items() if k != 'codes'}, 'frames': len(r['codes'])}
...
def load_paired_initialization(model, cache, cache_path, config):
    ...
    if file_sha(cache_path) != expected['cache_sha256'] or record_signature(cache) != protocol['record_signature']:
