#!/usr/bin/env python3
"""Train the archived successful FFHQ-v4 compound code with official Church dimensions and dynamics."""
import argparse
import codecs
import gc
import json
...
from src.church_ffhq_archived import (ChurchAux, make_prior, initialization_audit, FullBatchEpochStream,
...
def main():
    ...
    aux = ChurchAux(args.stage1, 16384, 2048, 3., coeff_scales=scales,
    ...
    config = json.loads(json.dumps({**vars(args), 'parameters': sum(p.numel() for p in model.parameters()),
        ...
        'coeff_scales': scales, 'coeff_max': 3., 'coeff_target_temperature': .5,
        'coeff_targets': 'normalized soft distributions with sampled contexts, directly from archived code',
        ...
        'objective': 'archived compound_objective: atom weight1.5, normalized soft coefficients, geometry .05 delayed2/ramp3 epochs',
    ...
    if args.wandb_id:
        ...
        wb = wandb.init(entity='helloimlixin-rutgers', project='laser', id=args.wandb_id,
            name=f'Church ARCHIVED FFHQ v4 405M {args.variant} SCRATCH batch256', group='church-ffhq-archived-20260911',
