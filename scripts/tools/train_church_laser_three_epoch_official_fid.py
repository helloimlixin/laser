#!/usr/bin/env python3
"""Fresh LASER prior after three Church tokenizer fine-tuning epochs.
...
from src.original_rq_training import (atomic_json,file_sha256,state_sha256,seed_all,
    load_stage2_config,fresh_transformer,CachedLatents)
...
from church_official_fid import evaluate_samples
...
def main():
    ...
    assert cache['stage1_epochs']==3 and not cache['smoke_only']
    ...
    config.vqvae.stage1_finetune_epochs=3
    ...
    config.evaluation=dict(fid_backend='rqvae.metrics.fid.compute_fid',
        input='saved float32 RGB pickle files',inception_batch_size=100,
        retain='generated codes, Inception activations/statistics, RGB hashes',
        rng_isolated=True,source='released immutable RQ-VAE repository')
    config.training_precision='FP16 model autocast; FP32 codebook geometry and cross entropy'
    if rank==0:
        (out/'official-fid-adapter.py').write_text((ROOT/'scripts/tools/church_official_fid.py').read_text())
    run=None
    if rank==0:
