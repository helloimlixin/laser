#!/usr/bin/env python3
"""Matched sampler sweep for a frozen LSUN-Church support-first model."""
...
from src.training.rqtransformer import (  # noqa: E402
    LaserAux,
    build_model,
...
def set_matched_seed(seed: int):
    # Each rank needs a distinct stream for unconditional generation, while
    # every sampler setting must see the same stream assigned to that rank.
...
def main():
    ...
    coefficient_patterns = cache_payload["coefficient_patterns"]
    expected_cache = {
        "format": "laser_coefficient_patterns_v1",
        "dataset": "lsun_church",
        "num_atoms": 16_384,
        "shape": [8, 8, 4],
        "coefficient_pattern_vocab_size": 4_096,
    }
    ...
    expected_checkpoint = {
        ...
        "num_atoms": 16_384,
        "sparsity_level": 4,
        "support_first_pattern_tokens": True,
    ...
    aux = LaserAux(
        args.stage1_checkpoint,
        num_atoms=16_384,
        coeff_vocab_size=int(checkpoint_config.get("coeff_vocab_size", 2_048)),
    ...
    del cache_payload, coefficient_patterns

    model = build_model(
        16_384 + int(checkpoint_config.get("coeff_vocab_size", 2_048)),
        16_384,
        compound=False,
        levelwise_var=False,
        support_first_patterns=True,
        coefficient_pattern_vocab_size=4_096,
        sparsity_level=4,
        model_preset="lsun-church-350m",
    )
