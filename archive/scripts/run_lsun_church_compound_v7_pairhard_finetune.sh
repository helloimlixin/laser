...
STAGE2_INIT="$ROOT/outputs/lsun-church-a16384-k4-causal-prefix-v6-aw15-full-e300-seed0-20260817/checkpoints/best_fid_21.1160_epoch_045.pt"
...
OUTPUT="${RUN_ROOT:-$ROOT/outputs/lsun-church-a16384-k4-compound-v7-pairhard-ft15-seed0-20260818}"
WANDB_ID="${WANDB_RUN_ID:-lsunchurchcmpv7pairhard-ft15-seed0-20260818}"
...
for required in "$PYTHON_BIN" "$STAGE1" "$STAGE2_INIT" "$DATA" "$CACHE" \
  "$FID_STATS" "$CONTINUOUS_RFID" "$QUANTIZED_RFID"; do
...
exec "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node="$NPROC" \
  ...
  --geometry-loss-weight 0.05 --geometry-start-epoch 0 --geometry-warmup-epochs 0 \
  ...
  --wandb-name "lsun-church-compound-v7-pairhard-ft15-seed0-20260818"
