#!/usr/bin/env python3
"""Pause the rejected ordering pilot, then launch a verified interleaved prior."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'outputs/church-interleaved-pattern-20260911')
    args=p.parse_args()
    output=args.output.resolve();output.mkdir(parents=True,exist_ok=True)
    if (output/'train').exists(): raise FileExistsError(output/'train')
    sources={}
    for pid in set(args.pause_pids+[args.credential_pid]):
        ...
        assert str(ROOT/'scripts/train_church_pattern_order.py') in command
    ...
    for pid in args.pause_pids:
        ...
        del saved
    (output/'previous-ordering-pause.json').write_text(json.dumps(list(sources.values()),indent=2)+'\n')
    ...
    assert digest(Path(calibration['selected_codebook']))==checked['codebook_sha256']
    run_id='church-interleaved-pattern-20260911'
    command=[sys.executable,'-u',str(ROOT/'scripts/train_church_interleaved_pattern.py'),
             '--output',str(output/'train'),'--wandb-id',run_id]
    env={**os.environ,'WANDB_API_KEY':credential,'CUDA_VISIBLE_DEVICES':'0','OMP_NUM_THREADS':'8',
    ...
    with (output/'train.log').open('ab') as f:
    ...
    result={'pid':child.pid,'gpu':0,'command':command,'started_unix':time.time(),
