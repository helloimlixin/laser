...
def persistent_checkpoint_dir(output: Path, configured: Path | None) -> Path:
    """Resolve checkpoint storage and reject unsupported ephemeral targets."""
    target = (configured or (output / "checkpoints")).expanduser().resolve()
    allowed_roots = [
        root.resolve()
        for root in (Path("/workspace"), Path("/scratch"), Path("/mnt/scratch"))
        if root.is_dir()
    ]
    if allowed_roots and not any(target.is_relative_to(root) for root in allowed_roots):
        raise ValueError(
            "Checkpoint directory must be under /workspace, /scratch, or /mnt/scratch; "
            f"got {target}"
        )
    return target
...
def create_warmup_cosine_lr_scheduler(
    ...
    *,
    ...
    """Create a resumable linear-warmup/cosine scheduler."""
    if not 0 < warmup_steps < total_steps:
        raise ValueError("warmup steps must be positive and below total schedule steps")
    if not 0 <= completed_steps <= total_steps:
        raise ValueError(
            f"completed optimizer steps ({completed_steps}) must be in [0, {total_steps}]"
        )
    ...
    def lr_multiplier(step: int) -> float:
        ...
        progress = min(max(float(step - warmup_steps + 1) / cosine_steps, 0.0), 1.0)
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        ...
        last_epoch=int(completed_steps) - 1 if state_dict is None else -1,
    )
    if state_dict is not None:
        ...
        for param_group, current_lr in zip(optimizer.param_groups, scheduler.get_last_lr()):
            param_group["lr"] = current_lr
    return scheduler


def release_cuda_memory(model=None):
    if model is not None and hasattr(model, "init_cache"):
        try:
            model.init_cache()
        except Exception:
            pass
...
class FlatImageDataset(Dataset):
    """A deterministic flat image directory used by the 70K-image FFHQ release."""
    ...
    def __init__(self, root: Path, transform):
        ...
        if not self.paths:
            raise ValueError(f"No images found directly under {self.root}")
...
def sample_class_grid(model, aux, class_names, output_dir: Path, step: int, wb=None,
                      num_classes=1000, atom_temperature=1.0, atom_top_k=None,
                      atom_top_p=0.92,
                      coeff_temperature=1.0, coeff_top_p=0.92):
    device = next(model.parameters()).device
    if num_classes == 1:
        chosen = torch.zeros(8, device=device, dtype=torch.long)
        labels = torch.zeros(64, device=device, dtype=torch.long)
    else:
        chosen = torch.randperm(num_classes, device=device)[:8]
    ...
    for index, axis in enumerate(axes.flat):
        ...
        axis.axis("off")
    prefix = "Unconditional" if num_classes == 1 else "Class-conditional"
    fig.suptitle(f"{prefix} samples — optimizer step {step}", fontsize=16)
    ...
    if wb is not None:
        import wandb
        wb.log({
            "samples/grid_8x8": wandb.Image(str(target)),
            "train/global_step": step,
        })
    plt.close(fig)
    if was_training:
        model.train()
    del images, labels, labels_cpu, chosen
    if "tokens" in locals():
        del tokens, partial
    if "atoms" in locals():
        del atoms, coeff_ids
...
def evaluate_generation_metrics(model, aux, val_loader, num_samples: int, batch_size: int = 64,
                                num_classes=1000, atom_temperature=1.0,
                                atom_top_k=None, atom_top_p=0.92,
                                coeff_temperature=1.0, coeff_top_p=0.92):
    from torchmetrics.image.fid import FrechetInceptionDistance
    from torchmetrics.image.inception import InceptionScore

    was_training = model.training
    fid_metric = inception_metric = None
    images = labels = local_indices = partial = tokens = atoms = coeff_ids = None
    try:
        device = next(model.parameters()).device
        ...
        while generated < local_samples:
            ...
            labels = (local_indices * world + process_rank).remainder(num_classes)
            if isinstance(model, CompoundLaserRQTransformer):
                atoms, coeff_ids = model.sample_compound(
...
def atomic_torch_save(payload, target: Path):
    ...
    try:
        ...
        # A failed save must not leave another checkpoint-sized file behind.
...
def upload_checkpoints(wb, paths: list[Path], *, artifact_name: str, aliases, metadata):
    ...
    for path in paths:
        artifact.add_file(
            str(path), name=path.name, policy="immutable", skip_cache=True
        )
    # Wait for the upload so checkpoint-sized staging copies cannot overlap
    # across epochs and grow without bound.
    wb.log_artifact(artifact, aliases=list(aliases)).wait()
...
def build_model(total_vocab_size: int, num_atoms: int, *, compound=False,
                ...
                architecture="imagenet-1400m", num_classes=1000):
    if architecture == "imagenet-1400m":
        embed_dim, body_layers, head_layers, heads = 1536, 42, 6, 24
    elif architecture == "ffhq-350m":
        # Original FFHQ RQ-Transformer 350M recipe: 1024-wide body-24/head-4.
        embed_dim, body_layers, head_layers, heads = 1024, 24, 4, 16
    else:
    ...
    cfg = OmegaConf.create({
        ...
        "vocab_size_cond": num_classes, "block_size_cond": 1,
        "body": {"n_layer": body_layers, "block": {"n_head": heads}},
        "head": {"n_layer": head_layers, "block": {"n_head": heads}},
    })
    if compound:
...
def main():
    ...
    p.add_argument("--dataset", choices=("imagenet", "ffhq-flat"), default="imagenet")
    p.add_argument(
        "--architecture", choices=("imagenet-1400m", "ffhq-350m"),
    ...
    p.add_argument("--coeff-scales", type=float, nargs=2, default=None,
                   help="Per-depth coefficient scales; overrides --coeff-scale")
    p.add_argument("--stage1-attn-resolutions", type=int, nargs="+", default=[8])
    p.add_argument(
    ...
    p.add_argument("--atom-top-k", type=int, default=0,
                   help="0 uses the complete atom vocabulary")
    ...
    p.add_argument("--lr-warmup-epochs", type=float, default=0.0)
    ...
    p.add_argument("--save-ckpt-freq", type=int, default=1,
    ...
    if args.lr_warmup_epochs >= args.lr_schedule_epochs:
        raise ValueError("--lr-warmup-epochs must be below --lr-schedule-epochs")
    ...
    if args.atom_top_k < 0:
        raise ValueError("--atom-top-k cannot be negative")
    ...
    if args.dataset == "ffhq-flat":
        image_dataset = FlatImageDataset(args.data, transform=image_transform())
        val_dataset = FlatImageDataset(args.data, transform=val_image_transform())
        class_names = ["unconditional"]
        num_classes = 1
        if args.architecture != "ffhq-350m":
            raise ValueError("FFHQ launches must use --architecture ffhq-350m")
    else:
    ...
    if args.token_cache:
        ...
        expected_format = "laser_compound_pairs_v1" if args.compound_tokens else None
        if expected_format and cache_meta.get("format") != expected_format:
            raise ValueError(
                f"Token cache format {cache_meta.get('format')!r} is not {expected_format!r}"
            )
        if int(cache_meta.get("num_atoms", args.num_atoms)) != args.num_atoms:
            raise ValueError("Token cache atom vocabulary does not match --num-atoms")
        if int(cache_meta.get("coeff_vocab_size", args.coeff_vocab_size)) != args.coeff_vocab_size:
            raise ValueError("Token cache coefficient vocabulary does not match")
        if float(cache_meta.get("coeff_max", args.coeff_max)) != args.coeff_max:
            raise ValueError("Token cache coefficient range does not match --coeff-max")
        if args.coeff_scales is None and cache_meta.get("coeff_scales") is not None:
            args.coeff_scales = [float(value) for value in cache_meta["coeff_scales"]]
        if args.dataset == "ffhq-flat" and cache_meta.get("validation_passed") is not True:
            raise ValueError("Refusing an unvalidated compound token cache")
    ...
    aux = LaserAux(args.checkpoint, args.num_atoms, args.coeff_vocab_size,
                   ...
                   attn_resolutions=args.stage1_attn_resolutions,
                   coeff_scales=args.coeff_scales).to(device)
    model = build_model(
    ...
    if use_wandb:
        ...
        wb = wandb.init(project=args.wandb_project, name=args.wandb_name,
                        ...
                        config={**vars(args), "architecture": (
                            f"compound-v4-micro{args.compound_micro_transformer_layers}-rqtransformer-{args.architecture}"
                            if args.compound_tokens and args.compound_micro_transformer_layers > 0
                            else f"compound-v3-refiner{args.compound_refiner_layers}-rqtransformer-{args.architecture}"
                            if args.compound_tokens and args.compound_refiner_layers > 0
                            else f"compound-rqtransformer-{args.architecture}" if args.compound_tokens
                            else f"official-rqtransformer-{args.architecture}"
                        ),
    ...
    if args.resume and resume_checkpoint.is_file():
        ...
        # Checkpoint payloads may have moved to a new run/node. Rebase retained
        # paths into this run and drop files that were not restored alongside
        # the recovery checkpoint.
        best_fid = [
            (float(x[0]), str(checkpoint_dir / Path(x[1]).name))
            for x in resume_payload.get("best_fid", [])
            if (checkpoint_dir / Path(x[1]).name).is_file()
        ]
        best_inception = [
            (float(x[0]), str(checkpoint_dir / Path(x[1]).name))
            for x in resume_payload.get("best_inception", [])
            if (checkpoint_dir / Path(x[1]).name).is_file()
        ]
        best_inception_score = [
            (float(x[0]), float(x[1]), int(x[2]), int(x[3]))
            for x in resume_payload.get("best_inception_score", [])
        ]
    ...
    if args.lr_schedule == "cosine":
        ...
        warmup_steps = round(args.lr_warmup_epochs * optimizer_steps_per_epoch)
        ...
        scheduler_factory = (
            create_warmup_cosine_lr_scheduler if warmup_steps
            else create_cosine_lr_scheduler
        )
        scheduler_kwargs = {
            "initial_lr": args.lr,
            "min_lr": args.min_lr,
        ...
        scheduler = scheduler_factory(optimizer, **scheduler_kwargs)
        if rank() == 0:
    ...
    for epoch in range(start_epoch, args.epochs):
                ...
                if args.save_step_freq > 0 and global_step % args.save_step_freq == 0:
                    ...
                    if rank() == 0:
                        recovery_snapshot = {
                            ...
                            "best_inception_score": best_inception_score,
                        }
                ...
                if args.sample_grid_every > 0 and global_step % args.sample_grid_every == 0:
                    ...
                    if rank() == 0:
                        target = sample_class_grid(
                            ...
                            atom_top_k=args.atom_top_k or args.num_atoms,
        ...
        if rank() == 0:
            ...
            inception_reference = best_inception or best_inception_score
            qualifies_inception = inception_score is not None and (
                len(inception_reference) < 3
                or inception_score > min(x[0] for x in inception_reference)
            )
            best_path = None
            if qualifies:
            ...
            if qualifies_inception:
                ...
                while len(best_inception) > 3:
                    ...
                    if stale_path.is_file():
                        stale_path.unlink()
                best_inception_score.append((
                    inception_score, inception_score_std, epoch + 1, global_step,
                ))
                best_inception_score.sort(key=lambda item: item[0], reverse=True)
                del best_inception_score[3:]
            if save_epoch:
                snapshot = {
                ...
                # version contains the recovery checkpoint and all locally
                # available members of the top-three FID/IS sets.
                if args.upload_checkpoints and run_fid:
                    artifact_aliases = ["latest", "last", f"epoch-{epoch + 1}"]
                    if best_path is not None:
                        best_rank = next(
                            index for index, (_, path) in enumerate(best_fid, start=1)
                            if path == str(best_path)
                        )
                        artifact_aliases.extend([
                            "best-fid", f"best-fid-{best_rank}", f"fid-{fid:.4f}",
                        ])
                        if best_rank == 1:
                            artifact_aliases.append("best")
                    if qualifies_inception:
                        artifact_aliases.extend(["best-is", f"is-{inception_score:.4f}"])
                    ...
                    upload_checkpoints(
                        ...
                        metadata={
                            ...
                            "inception_score": inception_score,
                            "inception_score_std": inception_score_std,
                            "is_top3_fid_checkpoint": best_path is not None,
                            "is_top3_inception_score_checkpoint": qualifies_inception,
                            "best_fid_top3": [
                                {"fid": item_fid, "path": item_path}
                                for item_fid, item_path in best_fid
                            ],
                            "best_inception_score_top3": [
                                {
                                    "inception_score": item_score,
                                    "inception_score_std": item_std,
                                    "epoch": item_epoch,
                                    "step": item_step,
                                    "artifact_alias": f"epoch-{item_epoch}",
                                }
                                for item_score, item_std, item_epoch, item_step
                                in best_inception_score
                            ],
