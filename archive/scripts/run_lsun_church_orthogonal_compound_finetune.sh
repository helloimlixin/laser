#!/usr/bin/env bash
# Causal ordered-orthogonal coordinate fine-tune for LSUN Church.
...
SOURCE_CACHE="$BASELINE_ROOT/token_cache/lsun_church_train_a16384k4_compound_pairs.pt"
DATA="$BASELINE_ROOT/data/lsun"
FID_STATS="$ROOT/third_party/rq-vae-transformer/assets/fid_stats/lsun_256_church.npz"
RUN_ROOT="${RUN_ROOT:-$ROOT/outputs/lsun-church-a16384-k4-orthogonal-stage2-20260822}"
ORTHOGONAL_CACHE="$RUN_ROOT/token_cache/lsun_church_train_a16384k4_orthogonal_compound.pt"
CONTINUOUS_RFID="$RUN_ROOT/token_cache/rfid_lsun_church_train126227_orthogonal_continuous.json"
QUANTIZED_RFID="$RUN_ROOT/token_cache/rfid_lsun_church_train126227_orthogonal_quantized.json"
OUTPUT="$RUN_ROOT/stage2"
WANDB_ID="${WANDB_RUN_ID:-lsunchurcha16384k4orthogonal-20260822}"
...
if [[ ! -f "$ORTHOGONAL_CACHE" ]]; then
  status "$active_phase" starting "convert 126227 cached sparse codes"
...
run_rfid() {
  ...
  status "$active_phase" starting "126227 paired reconstructions"
  "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node=1 \
    ...
    --output "$output" --num-images 126227 --batch-size 256 \
...
if [[ ! -f "$RUN_ROOT/.smoke_complete" ]]; then
  status "$active_phase" starting "pair-hard transfer plus one optimizer step"
  "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node=1 \
    ...
    --distributed-backend ddp --epochs 50 --batch-size 32 --total-batch-size 32 \
    --max-optimizer-steps 1 --smoke-test --lr 0.0001 \
    --lr-schedule cosine --lr-schedule-epochs 50 --min-lr 0.00001 \
...
status "$active_phase" starting "50-epoch Church 350M orthogonal fine-tune"
init_args=()
...
exec "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node=1 \
  ...
  "${init_args[@]}" \
  --distributed-backend ddp --epochs 50 --batch-size 32 --total-batch-size 256 \
  --lr 0.0001 --lr-schedule cosine --lr-schedule-epochs 50 --min-lr 0.00001 \
  ...
  --wandb-name "lsun-church-orthogonal-compound-ft50-20260822" \
