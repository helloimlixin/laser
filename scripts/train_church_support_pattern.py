#!/usr/bin/env python3
"""Train a calibrated support-first Church prior with joint coefficient patterns."""
import argparse
import codecs
import gc
import json
...
from src.church_support_pattern_training import support_pattern_prior, pattern_objective
...
def evaluate(model, aux, data, args):
    ...
    for first in range(0, len(data['atoms']), 16):
        ...
        _, metrics = pattern_objective(model, aux, atoms, physical)
        ...
        for first in range(0, count, args.generation_batch):
            ...
            with torch.autocast('cuda', dtype=torch.bfloat16):
                atoms, ids = model.sample_compound(batch, aux,
                    ...
                    coeff_top_p=None, atom_temperature=1.,
...
def main():
    ...
    p.add_argument('--minimum-stop-epoch', type=int, default=10)
    ...
    p.add_argument('--coeff-top-p', type=float, choices=[1.], default=1.,
                   help='This calibrated hard-pattern pilot uses the full joint coefficient distribution')
    ...
    model = support_pattern_prior(size, args.dropout).cuda()
    ...
    config = json.loads(json.dumps({**vars(args), 'architecture': 'balanced',
        ...
        'objective': '(1.5 sum of four atom NLLs + 4 joint pattern NLL)/10',
        'depth_context': 'deterministic sequential prefix sums of selected atom vectors',
        ...
        'source_hashes': {name: sha256_file(ROOT / name) for name in [
            'scripts/train_church_support_pattern.py', 'src/church_support_pattern_training.py',
    ...
    if args.wandb_id:
        ...
        wb = wandb.init(entity='helloimlixin-rutgers', project='laser', id=args.wandb_id,
            name=args.wandb_id, group='church-support-pattern-20260911', job_type='support-pattern-stage2-scratch',
    ...
    def log(row):
        ...
        write_json(args.output / 'status.json', {'pid': os.getpid(), 'target_mode': 'support_then_joint_pattern',
        ...
        while stream.epoch + stream.position / stream.size < args.epochs and not early_stop:
            for images, actual_indices in loader:
                ...
                for micro, batch in enumerate(images.split(args.microbatch)):
                    ...
                    loss, metrics = pattern_objective(model, aux, atoms.long(), physical)
